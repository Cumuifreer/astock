...
export function runSignalEvaluation(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
  return post('/api/backtest/signal-evaluation', payload);
}

export function getSignalEvaluation(runId: string): Promise<Record<string, unknown>> {
  return request<Record<string, unknown>>(`/api/backtest/signal-evaluation/${encodeURIComponent(runId)}`);
}

export function runPortfolioBacktest(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
  return post('/api/backtest/portfolio', payload);
}
