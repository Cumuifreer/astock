...
def test_intraday_scheduler_uses_light_slots_when_boards_are_disabled(tmp_path, monkeypatch):
    db = Database(tmp_path / "ashare_test.duckdb")
    migrate(db)
    service = UpdateService(db)
    started = []
    ...
    assert scheduler.tick(datetime(2026, 5, 22, 10, 5, tzinfo=ZoneInfo("Asia/Shanghai"))) is None

    task_id = scheduler.tick(datetime(2026, 5, 22, 10, 36, tzinfo=ZoneInfo("Asia/Shanghai")))

    rows = db.query("SELECT id, payload_json FROM task_runs WHERE kind = 'intraday'")
    assert task_id == "intraday-auto-20260522-1035"
    assert len(rows) == 1
    assert '"mode": "light_refresh"' in rows[0]["payload_json"]


def test_intraday_scheduler_uses_radar_slots_when_any_board_is_enabled(tmp_path, monkeypatch):
    db = Database(tmp_path / "ashare_test.duckdb")
    migrate(db)
    service = UpdateService(db)
    service.intraday_service.save_config({"enabled_boards": {"anomaly": True}})
...
def test_runtime_health_reports_dynamic_light_schedule_when_boards_are_disabled(tmp_path):
    db = Database(tmp_path / "ashare_test.duckdb")
    migrate(db)
    service = UpdateService(db)
    now = datetime(2026, 5, 22, 10, 4, tzinfo=ZoneInfo("Asia/Shanghai"))
    ...
    health = DataService(db).runtime_health(
        ...
        poll_seconds=30,
        catchup_minutes=8,
        schedule=service.intraday_schedule_text(),
        scheduler_mode=service.intraday_scheduler_mode(),
        enabled_boards=service.intraday_enabled_boards(),
    )

    assert health["scheduler"]["mode"] == "light_refresh"
    assert health["scheduler"]["enabled_boards"] == {"anomaly": False, "pullback": False, "risk": False}
    assert health["scheduler"]["slot_count"] < 25
    assert health["scheduler"]["next_slot"]["time"] == "10:35"
